...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const tool = {
    ...
    description: 'Generate a listening drill question for kids learning Spanish.',
    input_schema: {
      type: 'object',
      properties: {
        phrase: { type: 'string', description: `A natural Spanish phrase, ${wordCount} words. Age-appropriate for a child.` },
        translation: { type: 'string', description: 'English translation of the phrase.' },
        options: {
          type: 'array',
          items: { type: 'string' },
          description: '4 options: the correct Spanish phrase + 3 plausible distractors. Mix up word order or swap similar words.',
        },
  ...
  try {
    ...
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 400,
        system: `Generate listening drill content for children ages 7-14 learning Spanish. Topic: ${moduleId}. Keep vocabulary simple and age-appropriate.${avoidList ? ` Avoid repeating: ${avoidList}.` : ''}`,
        tools: [tool],
